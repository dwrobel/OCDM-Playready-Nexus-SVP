...
MediaKeySession::MediaKeySession(const uint8_t *f_pbInitData, uint32_t f_cbInitData, DRM_VOID *f_pOEMContext)
    ...
    {
        if (dr != 0) {
            printf("%d Expect platform to support Secure Clock or Anti-Rollback Clock.  Possible certificate error.%jd\n",
                   __LINE__, static_cast<intmax_t>(dr));
            goto ErrorExit;
        }
    }
...
#else
    printf("%s\n", "Playready 3.0 support of None-SVP, not implemented yet!");
    return CDMi_S_FALSE;
#endif
}
